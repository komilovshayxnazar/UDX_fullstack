import { Home, Video, PlusCircle, TrendingUp, User, FileText, ArrowLeftRight } from 'lucide-react';
...
export function BottomNavigation({ activeTab, onTabChange, mode, onSwitchRole }: BottomNavigationProps) {
  ...
  const tabs = mode === 'buyer' ? allTabs.filter(tab => tab.id !== 'add') : allTabs;
      ...
      <div className="flex items-center justify-around px-1 py-2">
        ...
        {mode && onSwitchRole && (
          <button
            ...
            className="flex flex-col items-center gap-1 rounded-lg px-2 py-2 transition-all text-emerald-600 hover:bg-emerald-50"
          >
            <ArrowLeftRight className="h-5 w-5" />
            <span className="text-[10px]">{mode === 'buyer' ? 'Seller' : 'Buyer'}</span>
          </button>
        )}
      </div>
    </div>
