...
async def _start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args:
        ...
        r = get_redis()
        if r:
            await r.setex(f"udx:tg:token:{token}", 300, str(chat_id))
...
async def _contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    r = get_redis()
    if r:
        await r.set(f"udx:tg:phone:{phone_hash}", str(chat_id))
    # Fayl fallback — ham saqlaymiz
...
async def get_chat_id_by_phone_hash(phone_hash: str) -> int | None:
    """Return chat_id for a given phone_hash, or None."""
    r = get_redis()
    if r:
        val = await r.get(f"udx:tg:phone:{phone_hash}")
        if val:
            return int(val)
...
async def send_otp_by_phone_hash(phone_hash: str, otp_code: str) -> bool:
    ...
    chat_id = await get_chat_id_by_phone_hash(phone_hash)
    if not chat_id:
...
async def get_chat_id(telegram_username: str) -> int | None:
    """Return stored chat_id for a given Telegram username, or None."""
    username = telegram_username.lower().lstrip("@")
    r = get_redis()
    if r:
        val = await r.get(f"udx:tg:username:{username}")
        if val:
            return int(val)
